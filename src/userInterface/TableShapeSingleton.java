...
public class TableShapeSingleton {
  ...
  private static TableShapeSingleton tableShapeSingleton;
  private StringBuilder[][] table;
  private int[] maxWidth;

  private TableShapeSingleton() {
  }

  public static TableShapeSingleton getInstance() {
    if (tableShapeSingleton == null) {
      tableShapeSingleton = new TableShapeSingleton();
    }
  ...
  private boolean isTypeString(String input){
  ...
  private String getStringValue(String input) {
    if (input.equals(new String("null"))) {
      return new String("");
    } else if (isTypeString(input)) {
      return input.substring(1, input.length() - 1);
    }
    return input;
  }
  
  public void setTable(LinkedList<LinkedList<String>> givenQuery) {
    int length = givenQuery.size();
    int width = 0;
    if (givenQuery.size() > 0) {
      width = givenQuery.get(0).size();
    }
    table = new StringBuilder[length][width];
    maxWidth = new int[width];
    for (int i = 0; i < length; i++) {
      for (int j = 0; j < width; j++) {
        table[i][j] = new StringBuilder(getStringValue(givenQuery.get(i).get(j)));
